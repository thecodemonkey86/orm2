...
import sunjava.cls.bean.method.MethodHasAddedManyToMany;
import sunjava.cls.bean.method.MethodHasRemovedManyToMany;
...
import sunjava.cls.instruction.ForeachLoop;
...
//		ifHasUpdate.thenBlock()._callMethodInstr(sqlQuery, ClsSqlQuery.METHOD_NAME_BEGIN_TRANSACTION);
		...
		for(Column col : columns) {
			if (col.isNullable()) {
				...
				ifColIsNull.elseBlock()._callMethodInstr(sqlQuery, 
						ClsSqlQuery.setValue,JavaString.stringConstant(col.getName()), 
						Types.SqlParam.callStaticMethod(ClsSqlParam.get, BeanCls.accessAttrGetterByColumn(pBean,col,false)));
				// end insert
				...
				ifIsInsertNew.thenBlock()._callMethodInstr(sqlQuery,  ClsSqlQuery.setValue,JavaString.stringConstant(col.getName()), Types.SqlParam.callStaticMethod(ClsSqlParam.get, BeanCls.accessAttrGetterByColumn(pBean,col,true)));
		...
		for (Column colPk : tbl.getPrimaryKey().getColumns()) {
			ifNotIsPkModified.thenBlock()._callMethodInstr(sqlQuery,  ClsSqlQuery.where,JavaString.stringConstant(colPk.getName()+"=?"), Types.SqlParam.callStaticMethod(ClsSqlParam.get, BeanCls.accessAttrGetterByColumn(pBean,colPk,true)));
			
			
			ifNotIsPkModified.elseBlock()._callMethodInstr(sqlQuery, 
					ClsSqlQuery.setValue,JavaString.stringConstant(colPk.getName()), 
					Types.SqlParam.callStaticMethod(ClsSqlParam.get, BeanCls.accessAttrGetterByColumn(pBean,colPk,true)));
			ifNotIsPkModified.elseBlock()._callMethodInstr(sqlQuery,  ClsSqlQuery.where,JavaString.stringConstant(colPk.getName()+"=?"), Types.SqlParam.callStaticMethod(ClsSqlParam.get, BeanCls.accessPrimaryKeyPreviousAttrGetterByColumn(pBean,colPk)));
		}
		...
		*/
		...
		if(tbl.isAutoIncrement()) {
			...
			ifRsAutoIncrementNext.thenBlock()._callMethodInstr(pBean, ClsBaseBean.METHOD_NAME_SET_AUTO_INCREMENT_ID, rsAutoIncrement.callMethod(ClsResultSet.METHOD_NAME_GET_INT, JavaString.stringConstant(tbl.getPrimaryKey().getFirstColumn().getName())));
		...
		for(ManyRelation r:manyRelations) {
			...
			Var varDeleteSql = ifRemoveBeans.thenBlock()._declare(Types.SqlQuery, "deleteSqlQuery",BeanCls.getSqlQueryCls().newInstance(aSqlCon));
			...
			Var varAddSql = ifAddedBeans.thenBlock()._declare(Types.SqlQuery, "addSqlQuery",BeanCls.getSqlQueryCls().newInstance(aSqlCon));
			...
			for(int i = 0; i < r.getSourceTable().getPrimaryKey().getColumnCount(); i++) {
				...
				addInsertRowArgs[i] = Types.SqlParam.callStaticMethod(ClsSqlParam.get, pBean.callAttrGetter(new Attr(BeanCls.getTypeMapper().columnToType(col), col.getCamelCaseName()))); 
			}
