...
import sunjava.cls.expression.Operators;
...
public class IfBlock extends Instruction{
	...
	public String toString() {
		...
		if (elseifCondition!=null) {
			for (int i = 0; i < elseifCondition.size(); i++) {
				sb.append("else if");
				sb.append(CodeUtil.parentheses(elseifCondition.get(i)));
	...
	public void collectImports(AbstractJavaCls cls) {
		...
		if (elseifCondition!=null) {
			for(Expression elif : elseifCondition ) {
				elif.collectImports(cls);
			}
			for (InstructionBlock elifInstr : this.elseifInstr) {
				elifInstr.collectImports(cls);
			}
		}
		
		if(elseInstr != null) {
			elseInstr.collectImports(cls);
		}
	}
}
