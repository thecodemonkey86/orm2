...
////			selectFields.add(Entities.get(r.getDestTable()).callStaticMethod("getSelectFields", JavaString.fromStringConstant(r.getAlias())));
////			//bCount++;
////		}
...
//			}
//			
//			exprQSqlQuery = exprQSqlQuery.callMethod("leftJoin", JavaString.fromExpression(Entities.get(r.getDestTable()).callStaticMethod("getTableName")),JavaString.fromStringConstant(r.getAlias()), JavaString.fromStringConstant(CodeUtil2.concat(joinConditions," AND ")));
//			//bCount++;
//		}
//		
//		
...
//								recDoWhile.callMethod("value", JavaString.fromStringConstant(r.getAlias()+"__"+ colPk.getName())).callMethod(EntityCls.getTypeMapper().getConvertMethod(colPk.getDbType())));
//					}
//				} else {
//					Column colPk=r.getDestTable().getPrimaryKey().getFirstColumn();
//					pkForeign = ifNotPkForeignIsNull.getIfInstr()._declare(entityPk, "pkForeignB"+r.getAlias(),recDoWhile.callMethod("value", JavaString.fromStringConstant(r.getAlias()+"__"+ r.getDestTable().getPrimaryKey().getFirstColumn().getName())).callMethod(EntityCls.getTypeMapper().getConvertMethod(colPk.getDbType())));
//				}
...
//					ifForeachPkCompare.getIfInstr().addInstr(varIfNotE1SetContainsForeachEntity.callMethodInstruction("set"+ col.getUc1stCamelCaseName()+"Internal",recDoWhile.callMethod("value", JavaString.fromStringConstant("e1__"+ col.getName())).callMethod(EntityCls.getTypeMapper().getConvertMethod(col.getDbType()))));
//				}
