...
public class MethodEntityLoad extends Method {
	...
	public MethodEntityLoad(EntityCls entity) {
		...
		this.entity = entity;
		pEntity = addParam(entity.toRawPointer(), "entity");
//		setConstQualifier(true);
	...
	public void addImplementation() {
		...
		for(OneToManyRelation r:oneToManyRelations) {
			...
			exprQSqlQuery = exprQSqlQuery.callMethod(r.isComposition() ? "join" : "leftJoin", QString.fromExpression(Entities.get(r.getDestTable()).callStaticMethod("getTableName")),QString.fromStringConstant(r.getAlias()), QString.fromStringConstant(CodeUtil2.concat(joinConditions," AND ")));
			if(r.hasAdditionalJoin()) {
				exprQSqlQuery = exprQSqlQuery.callMethod(ClsSqlQuery.join,QString.fromStringConstant(r.getAdditionalJoin()));
			}
		}
...
//				IfBlock ifNotContains = 
		...
		for(OneRelation r:oneRelations) {
			...
			try {
				...
				ifBlock.thenBlock()._callMethodInstr(pEntity, MethodOneRelationAttrSetter.getMethodName(r, true),  _this().callMethod(MethodGetFromRecord.getMethodName(foreignCls), rec, QString.fromStringConstant(r.getAlias())));
			} catch (Exception e) {
				e.printStackTrace();
			}
			//bCount++;
