			...
			try{
				...
				if (!col.hasOneRelation()) {
					...
					if (col.isNullable()) {
						...
						ifIsNull.elseBlock().addInstr(vBean.callMethodInstruction("set"+col.getUc1stCamelCaseName()+"Internal",getParam("record").callMethod("value", new QStringPlusOperatorExpression(getParam("alias"), QString.fromStringConstant("__"+ col.getName()))).callMethod(EntityCls.getDatabaseMapper().getQVariantConvertMethod(col))));
					} else {
						addInstr(vBean.callMethodInstruction("set"+col.getUc1stCamelCaseName()+"Internal",getParam("record").callMethod("value", exprArrayIndex).callMethod(EntityCls.getDatabaseMapper().getQVariantConvertMethod(col))));
					}
				}
