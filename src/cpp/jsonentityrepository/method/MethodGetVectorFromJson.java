...
public class MethodGetVectorFromJson extends Method{
	...
	public void addImplementation() {
		...
		for(Column col : entity.getTbl().getAllColumns()) {
			if(!col.isRelationDestColumn() || col.isPartOfPk()) {
				if( col.isNullable()) {
					...
					ifValueIsNull.thenBlock().addInstr( e1.callMethodInstruction(MethodColumnAttrSetterInternal.getMethodName(col), JsonOrmUtil.jsonConvertMethod(jsonobject.callMethod(ClsQJsonObject.value, QString.fromStringConstant(col.getName())), ((Optional)( ((Cls)e1.getType()).getAttrByName(col.getCamelCaseName())).getType()).getElementType())));
				} else {
		...
		for(OneRelation r : entity.getOneRelations() ) {
			...
			Var relationEntityData =ifValueIsNull.thenBlock()._declare(e.toSharedPtr(),r.getAlias(),parent.callStaticMethod(MethodGetOneFromJson.getMethodName(e), jsonobject.callMethod(ClsQJsonObject.value,QString.fromStringConstant(OrmUtil.getOneRelationDestAttrName(r))).callMethod(ClsQJsonValue.toObject)));
			ifValueIsNull.thenBlock().addInstr(e1.callMethodInstruction(MethodAttrSetterInternal.getMethodName(r), relationEntityData));
		}
		for(OneToManyRelation r : entity.getOneToManyRelations() ) {
			...
			JsonEntity e = JsonEntities.get(r.getDestTable());
			Var relationEntityDataArray  =ifValueIsNull.thenBlock()._declare(JsonTypes.QJsonArray,"_jsonDataArray" +r.getAlias(), jsonobject.callMethod(ClsQJsonObject.value,QString.fromStringConstant(OrmUtil.getOneToManyRelationDestAttrNameSingular(r))).callMethod(ClsQJsonValue.toArray));
		...
		for(ManyRelation r : entity.getManyRelations() ) {
			...
			JsonEntity e = JsonEntities.get(r.getDestTable());
			Var relationEntityDataArray  =ifValueIsNull.thenBlock()._declare(JsonTypes.QJsonArray,"_jsonDataArray" +r.getAlias(), jsonobject.callMethod(ClsQJsonObject.value,QString.fromStringConstant(OrmUtil.getManyRelationDestAttrName(r))).callMethod(ClsQJsonValue.toArray));
			ForeachLoop foreachRelationEntity = ifValueIsNull.thenBlock()._foreach(new Var(JsonTypes.QJsonValue.toConstRef(),"_jsonData" + r.getAlias()), relationEntityDataArray);
			foreachRelationEntity.addInstr(e1.callMethodInstruction(MethodAddRelatedEntityInternal.getMethodName(r), parent.callStaticMethod(MethodGetOneFromJson.getMethodName(e),foreachRelationEntity.getVar().callMethod(ClsQJsonValue.toObject))));
		}
